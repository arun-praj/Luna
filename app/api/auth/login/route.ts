...
import { issueUserEmailVerification } from "@/backend/auth/verification-delivery";
...
export async function POST(request: Request) {
  ...
  if (!user.emailVerifiedAt) {
    ...
    return NextResponse.json({ emailVerificationRequired: true, verificationToken: await createEmailVerificationToken(user.id), ...verificationEmailResponse(verification.delivery), message: "Please verify your email before signing in." });
  }
