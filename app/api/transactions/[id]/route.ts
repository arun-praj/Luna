...
export async function PATCH(request: Request, { params }: Context) { const userId = await requireAccessToken(request); const { id } = await params; if (!userId) return errorResponse("Authentication required", 401); const body = await request.json().catch(() => null); const receiptImageUrl = receiptReferenceFromBody(body); const [existing] = await db.select({ receiptImageUrl: transactions.receiptImageUrl }).from(transactions).where(and(eq(transactions.id, id), eq(transactions.userId, userId))).limit(1); const replaceableReceipt = receiptImageUrl && receiptImageUrl !== existing?.receiptImageUrl ? receiptImageUrl : null; const parsed = transactionInput.safeParse(body); if (!parsed.success) { await deleteUploadIfUnreferenced(userId, "transaction-receipts", replaceableReceipt); const titleIssue = parsed.error.issues.find((issue) => issue.path[0] === "title"); return errorResponse(titleIssue ? "Add a title for this transaction" : "Invalid transaction", 400); } try { const transaction = await updateTransaction(userId, id, parsed.data); if (transaction.receiptImageUrl) await attachStoredObject(userId, "transaction-receipts", transaction.receiptImageUrl, "transaction", id); await deleteUploadIfUnreferenced(userId, "transaction-receipts", existing?.receiptImageUrl !== parsed.data.receiptImageUrl ? existing?.receiptImageUrl : null); scheduleHomeAlertRepair(userId); return NextResponse.json({ transaction: serializeTransaction(transaction) }); } catch (error) { await deleteUploadIfUnreferenced(userId, "transaction-receipts", replaceableReceipt); return errorResponse(error instanceof Error ? error.message : "Unable to update transaction", 400); } }
export async function DELETE(request: Request, { params }: Context) { const userId = await requireAccessToken(request); const { id } = await params; if (!userId) return errorResponse("Authentication required", 401); const [existing] = await db.select({ receiptImageUrl: transactions.receiptImageUrl }).from(transactions).where(and(eq(transactions.id, id), eq(transactions.userId, userId))).limit(1); try { await deleteTransaction(userId, id); await deleteUploadIfUnreferenced(userId, "transaction-receipts", existing?.receiptImageUrl); scheduleHomeAlertRepair(userId); return NextResponse.json({ success: true }); } catch (error) { return errorResponse(error instanceof Error ? error.message : "Unable to delete transaction", 400); } }
