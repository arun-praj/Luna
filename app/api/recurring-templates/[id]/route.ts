...
import { recurringTemplateInput, recurringTemplatePatchInput } from "@/backend/domain/validation";
import { scheduleHomeAlertRepair } from "@/backend/domain/home-alert-service";

type Context = { params: Promise<{ id: string }> };
type RecurringTemplateInput = ReturnType<typeof recurringTemplateInput.parse>;

async function validatePatchReferences(userId: string, input: RecurringTemplateInput) {
  ...
  if (input.transferToAccountId) {
    ...
    if ((input.type === "transfer" || (input.type === "savings" && input.goalId)) && target.currency !== account.currency) {
      throw new Error("Recurring transfers must use the same currency");
    }
  }
  ...
  if (input.goalId) {
    ...
    if (input.transferToAccountId !== goal.accountId) throw new Error("Savings must move money into the selected goal account");
  }
}
...
export async function PATCH(request: Request, { params }: Context) {
  ...
  const parsed = recurringTemplatePatchInput.safeParse(await request.json().catch(() => null));
  if (!parsed.success || Object.keys(parsed.data).length === 0) return errorResponse("Invalid recurring template update", 400);

  try {
    const [current] = await db.select().from(recurringTemplates).where(and(eq(recurringTemplates.id, id), eq(recurringTemplates.userId, userId))).limit(1);
    if (!current) return errorResponse("Recurring template not found", 404);

    // Validate the complete post-update record, not just the submitted patch.
    // This keeps required fields intact across partial updates and type changes.
    const merged = recurringTemplateInput.safeParse({ ...current, ...parsed.data });
    if (!merged.success) return errorResponse("Invalid recurring template update", 400);
    await validatePatchReferences(userId, merged.data);
