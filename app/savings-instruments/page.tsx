    ...
    <main className="page-route-enter min-h-dvh bg-background">
      <div className="mx-auto w-full max-w-[720px] px-4 pb-12 sm:px-5">
        <StickyPageHeader className="-mx-4 px-4 pb-3 sm:-mx-5 sm:px-5">
          <PageHeader
            leading={<Link href={backHref} aria-label="Back" className="flex size-11 shrink-0 items-center justify-center rounded-[11px] border border-border bg-card text-foreground"><ArrowLeft aria-hidden="true" className="size-5" /></Link>}
            title={<div className="min-w-0"><p className="text-xs font-medium text-muted-foreground">Manage your money</p><h1 className="text-[26px] font-semibold tracking-[-0.04em]">Saving Instruments</h1></div>}
            secondary={<GuideIcon href={withReturnTo("/savings-instruments/guide", currentRoute)} label="Saving instruments" />}
            actions={<Link href={withReturnTo("/savings-instruments/new", currentRoute)} aria-label="Add saving instrument" className="flex size-11 shrink-0 items-center justify-center rounded-[11px] border border-primary/20 bg-primary-soft text-primary"><Plus aria-hidden="true" className="size-[19px]" /></Link>}
          />
        </StickyPageHeader>
