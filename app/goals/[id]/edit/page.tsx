...
type Goal = { id: string; name: string; targetAmount: number; monthlyContribution: number; targetDate: string | null; allocatedAmount: number; accountId: string | null };
...
export default function EditGoalPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    void Promise.all([authenticatedFetch(`/api/goals/${goalId}`), authenticatedFetch("/api/accounts")]).then(async ([goalResponse, accountResponse]) => {
      ...
      setMonthlyContribution(result.goal.monthlyContribution ? String(result.goal.monthlyContribution) : "");
