...
export function orderCategoryOptions<T extends CategoryOrderingOption>(options: T[], selectedCategoryId: string | null, transactionType: string) {
  ...
  const mostRecent = uniqueOptions.filter((option) => option.id !== selectedCategoryId && timestampValue(option.lastUsedAt) !== Number.NEGATIVE_INFINITY).sort((left, right) => timestampValue(right.lastUsedAt) - timestampValue(left.lastUsedAt))[0]?.id ?? null;
