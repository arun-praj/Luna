...
export function GoalActionForm({ goal, accounts, categories, onChanged }: { goal: Goal; accounts: Account[]; categories: Category[]; onChanged: () => void }) {
  ...
  const [setupInitial, setSetupInitial] = useState<string | null>(null);
  ...
  const setupSnapshot = JSON.stringify({ action, amount, accountId, categoryId, notes });
  const { requestDiscard, discardDialog } = useUnsavedChangesGuard(setupInitial !== null && setupSnapshot !== setupInitial);

  function openAction(nextAction: Action) {
    setAction(nextAction);
    const nextAccountId = spendableAccounts.find((account) => account.isDefault)?.id ?? spendableAccounts[0]?.id ?? "";
    const nextAmount = nextAction === "spend" ? String(goal.allocatedAmount) : "0";
    setAccountId(nextAccountId);
    setAmount(nextAmount);
    setCategoryId("");
    setNotes("");
    setError("");
    setSetupInitial(JSON.stringify({ action: nextAction, amount: nextAmount, accountId: nextAccountId, categoryId: "", notes: "" }));
    setSetupOpen(nextAction !== "contribute");
    setAmountOpen(nextAction === "contribute");
  }

  function closeSetup() {
    requestDiscard(() => {
      setSetupOpen(false);
      setSetupInitial(null);
  ...
  async function submit(nextAmount: string) {
    ...
    try {
      ...
      notifyTransactionsChanged(); setAmountOpen(false); setAction(null); setSetupInitial(null); onChanged();
  ...
  const contributionTopContent = action === "contribute" ? <div className="space-y-3"><div className="rounded-[18px] border border-primary/15 bg-primary-soft/45 p-4"><p className="text-sm font-semibold">{actionDescription}</p><div className="mt-3 flex items-center gap-2"><FlowAccount account={selectedAccount} label="Spendable" /><ArrowRight aria-hidden="true" className="size-4 shrink-0 text-primary" /><FlowAccount account={goalAccount} label="Goal account" /></div><p className="mt-3 text-xs leading-5 text-muted-foreground">The two balances and the goal progress update together.</p></div><fieldset className="min-w-0"><legend className="mb-2 px-1 text-xs font-semibold text-muted-foreground">Choose the account to take from</legend><div className="-mx-1 flex min-w-0 gap-2 overflow-x-auto overscroll-x-contain px-1 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{spendableAccounts.map((account) => <button key={account.id} type="button" aria-pressed={account.id === accountId} onClick={() => setAccountId(account.id)} className={`flex min-h-14 shrink-0 items-center gap-2 rounded-[12px] border px-2.5 pr-3 text-left transition-colors ${account.id === accountId ? "border-primary bg-primary-soft text-primary" : "border-border bg-card"}`}><span className="flex size-9 shrink-0 overflow-hidden rounded-[9px]"><AccountAvatar icon={account.icon} name={account.name} type={account.type} backgroundColor={account.backgroundColor} size={36} /></span><span className="flex min-w-0 flex-col items-start leading-tight"><span className="max-w-[145px] truncate text-xs font-semibold">{account.name}</span><span className="mt-1 text-[10px] font-medium tabular-nums text-muted-foreground">{account.currency} {amountLabel(account.currentBalance)}</span></span>{account.id === accountId ? <span className="flex size-5 shrink-0 items-center justify-center rounded-full bg-foreground text-background"><Check aria-hidden="true" className="size-3.5 stroke-[3]" /></span> : null}</button>)}</div></fieldset><label className="block"><span className="mb-1.5 block text-xs font-semibold text-muted-foreground">Note (optional)</span><input value={notes} onChange={(event) => setNotes(event.target.value)} maxLength={500} placeholder="Add a note" className="h-11 w-full rounded-[11px] border border-border bg-card px-3 text-sm font-medium outline-none placeholder:text-muted-foreground focus:border-primary focus:ring-2 focus:ring-primary/20" /></label></div> : undefined;
  ...
  return <>
    ...
    {setupTransition.mounted && action && action !== "contribute" ? <div role="presentation" className={`fixed inset-0 z-[60] flex items-end bg-foreground/25 backdrop-blur-[2px] ${setupTransition.closing ? "drawer-scrim-exit" : "drawer-scrim-enter"}`} onMouseDown={(event) => { if (event.target === event.currentTarget) closeSetup(); }}><section role="dialog" aria-modal="true" aria-labelledby="goal-action-setup-title" className={`max-h-[92dvh] w-full overflow-y-auto rounded-t-[24px] border-t border-border bg-background px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3 shadow-[0_-18px_50px_rgb(23_32_29_/_0.18)] ${setupTransition.closing ? "drawer-exit" : "drawer-enter"}`}><div className="mx-auto w-full max-w-[520px]"><div className="mx-auto mb-2 h-1.5 w-12 rounded-full bg-foreground/15" aria-hidden="true" /><header className="flex items-center justify-between gap-3 pb-4"><button type="button" onClick={closeSetup} aria-label="Close goal action setup" className="flex size-11 items-center justify-center rounded-[11px] border border-border bg-card text-muted-foreground"><X aria-hidden="true" className="size-5" /></button><div className="min-w-0 text-center"><p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-primary">{goal.name}</p><h2 id="goal-action-setup-title" className="mt-0.5 text-lg font-semibold tracking-[-0.03em]">{actionTitle}</h2></div><button type="button" onClick={continueToDialer} disabled={!isReady} className="rounded-[11px] bg-primary-soft px-3 py-2 text-sm font-semibold text-primary disabled:opacity-40">Enter amount</button></header>
        ...
        {action !== "spend" ? <fieldset className="mt-4 min-w-0"><legend className="mb-2 px-1 text-xs font-semibold text-muted-foreground">Choose where returned money goes</legend><div className="-mx-4 flex min-w-0 gap-2 overflow-x-auto overscroll-x-contain px-4 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{spendableAccounts.map((account) => <button key={account.id} type="button" aria-pressed={account.id === accountId} onClick={() => setAccountId(account.id)} className={`flex min-h-14 shrink-0 items-center gap-2 rounded-[12px] border px-2.5 pr-3 text-left transition-colors ${account.id === accountId ? "border-primary bg-primary-soft text-primary" : "border-border bg-card"}`}><span className="flex size-9 shrink-0 overflow-hidden rounded-[9px]"><AccountAvatar icon={account.icon} name={account.name} type={account.type} backgroundColor={account.backgroundColor} size={36} /></span><span className="flex min-w-0 flex-col items-start leading-tight"><span className="max-w-[145px] truncate text-xs font-semibold">{account.name}</span><span className="mt-1 text-[10px] font-medium tabular-nums text-muted-foreground">{account.currency} {amountLabel(account.currentBalance)}</span></span>{account.id === accountId ? <span className="flex size-5 shrink-0 items-center justify-center rounded-full bg-foreground text-background"><Check aria-hidden="true" className="size-3.5 stroke-[3]" /></span> : null}</button>)}</div></fieldset> : <div className="mt-4 flex items-center gap-2 rounded-[12px] bg-surface-subtle px-3 py-2.5 text-xs text-muted-foreground"><Target aria-hidden="true" className="size-4 shrink-0 text-primary" />The goal account is the purchase reference.</div>}
        {action === "spend" ? <label className="mt-4 block"><span className="mb-1.5 block text-xs font-semibold text-muted-foreground">Category (optional)</span><select value={categoryId} onChange={(event) => setCategoryId(event.target.value)} className="h-11 w-full rounded-[11px] border border-border bg-card px-3 text-sm font-medium outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"><option value="">No category</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label> : null}
        ...
        <div className="mt-4 flex gap-2"><button type="button" onClick={continueToDialer} disabled={!isReady} className="flex min-h-12 min-w-0 flex-1 items-center justify-between rounded-[12px] border border-border bg-card px-3 text-left transition-colors hover:border-primary/45 disabled:opacity-40"><span className="min-w-0"><span className="block text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">Amount</span><span className="mt-1 block truncate text-sm font-semibold tabular-nums">{selectedAccount?.currency ?? goalAccount?.currency ?? "NPR"} {amountLabel(Number(amount || 0))}</span></span><ArrowDownLeft aria-hidden="true" className="size-4 shrink-0 text-primary" /></button><button type="button" onClick={() => void submit(amount)} disabled={!isReady || Number(amount) <= 0 || saving} className="flex min-h-12 flex-1 items-center justify-center gap-2 rounded-[12px] bg-primary px-3 text-sm font-semibold text-primary-foreground shadow-sm transition-colors hover:bg-primary-hover disabled:cursor-not-allowed disabled:opacity-45">{saving ? "Adding…" : "Add funds"}<Check aria-hidden="true" className="size-4" /></button></div>
      </div></section></div> : null}

    <MoneyEditor open={amountOpen} value={amount} title={actionTitle} currency={selectedAccount?.currency ?? goalAccount?.currency ?? "NPR"} topContent={contributionTopContent} confirmPlacement="bottom" confirmLabel={action === "contribute" ? "Add funds" : "Done"} confirmDisabled={(value) => saving || !Number.isFinite(Number(value)) || Number(value) <= 0 || (action === "contribute" && !isReady) || (maxAmount !== undefined && Number(value) > maxAmount + 0.000001)} confirmValidation={(value) => maxAmount !== undefined && Number(value) > maxAmount + 0.000001 ? `Enter ${amountLabel(maxAmount)} or less.` : ""} liveValidation={action === "contribute" ? () => error : undefined} cancelVariant="text" cancelLabel="Cancel" onCancel={() => action === "contribute" ? requestDiscard(() => { setAmountOpen(false); setAction(null); setSetupInitial(null); }) : setAmountOpen(false)} onSet={(nextAmount) => { if (action === "contribute") void submit(nextAmount); else { setAmount(nextAmount); setAmountOpen(false); } }} />
    {discardDialog}
  </>;
}
